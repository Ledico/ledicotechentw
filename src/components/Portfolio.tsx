  ...
  const loadProjects = async () => {
    try {
      ...
      const projectsWithTags = (dbProjects || []).map((project: any) => ({
        ...
        linkTo: project.slug_enabled && project.slug ? `/${project.slug}` : undefined,
              ...
              <div className="relative overflow-hidden">
                ...
                {project.status && project.status !== 'draft' && (
                  <div className="absolute top-4 right-4 z-20">
                    <div className={`relative inline-flex items-center gap-1 px-2 py-1 md:px-3 md:py-1.5 rounded-full text-xs font-semibold backdrop-blur-sm border shadow-lg transition-all duration-300 ${
                      project.status === 'published'
                        ? 'bg-green-500/95 text-white border-green-400 shadow-green-500/50' :
                      project.status === 'ongoing'
                        ? 'bg-blue-500/95 text-white border-blue-400 shadow-blue-500/50' :
                      project.status === 'on_hold'
                        ? 'bg-amber-500/95 text-white border-amber-400 shadow-amber-500/50' :
                      project.status === 'planned'
                        ? 'bg-slate-500/95 text-white border-slate-400 shadow-slate-500/50' :
                        'bg-slate-400/95 text-white border-slate-300 shadow-slate-400/50'
                    }`}>
                      <span className="font-bold">
                        {project.status === 'published' ? 'Abgeschlossen' :
                         project.status === 'ongoing' ? 'Laufend' :
                         project.status === 'on_hold' ? 'Pausiert' :
                         project.status === 'planned' ? 'In Planung' :
                         project.status}
                      </span>
                    </div>
                  </div>
