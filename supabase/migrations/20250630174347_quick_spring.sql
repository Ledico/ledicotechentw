...
-- Create a function to check user status (for debugging) - Fixed type casting
...
BEGIN
  ...
  SELECT 
    ...
    au.id as user_id,
    au.email::text as email,  -- Explicit cast to text
...
-- Simple status check without function calls to avoid display issues
DO $$
DECLARE
  auth_count bigint;
